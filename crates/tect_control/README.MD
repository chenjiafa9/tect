...
impl Plugin for ObjectInteractionPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_systems(Update, (
                interaction_preview_system,run,
...
fn interaction_preview_system(
    ...
    let Ok(tool) = player_tool.get_single() else { return; };

    let hit = pointers.iter()
        .filter_map(|i| i.get_nearest_hit())
        .min_by_key(|(_, h)| (h.distance * 1000.0) as u32);

    let Some((hit_entity, hit_pos, hit_normal)) = hit else { return; };

    // 情况1：手持斧头 + 指向可破坏物 → 红色大 X
    if *tool == PlayerTool::Axe && destructible.contains(hit_entity) {
        let size = 1.0;
        gizmos.line(hit_pos + Vec3::new(-size, size, 0.0), hit_pos + Vec3::new(size, -size, 0.0), Color::srgb(1.0, 0.0, 0.0));
        gizmos.line(hit_pos + Vec3::new(-size, -size, 0.0), hit_pos + Vec3::new(size, size, 0.0), Color::srgb(1.0, 0.0, 0.0));
        gizmos.circle(hit_pos, Direction3d::Y, size * 0.8, Color::srgb(1.0, 0.2, 0.2));
        return;
    }
